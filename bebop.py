...
# this will be in new separate repository as common library fo robotika
...
class Bebop:
    ...
    def __init__(self, metalog=None, onlyIFrames=True):
        if metalog is None:
            self._discovery()
            metalog = MetaLog()
        self.navdata = metalog.createLoggedSocket(
            "navdata", headerFormat="<BBBI")
        ...
        self.buf = ""
        self.videoFrameProcessor = VideoFrames(
            onlyIFrames=onlyIFrames, verbose=False)
        self.videoCbk = None
        ...
        # Para recibir el evento e fin de movmiento relativo
        # Ver navdata.py lin~223
        self.movementState = None
    ...
    def hover(self):
        self.update(cmd=movePCMDCmd(
            active=True, roll=0, pitch=0, yaw=0, gaz=0))
    ...
    def trayectoria(self, trayectoria):
        fileInfo = open(
            "/home/david/Documents/drone/trayectoryinfo.data", "ab")
        self.takeoff()
        time.sleep(3)
        try:
            for i in range(0, trayectoria.shape[0]):
                #travel = trayectoria[i,:]-trayectoria[i-1,:]
                travel = trayectoria[i, :]
                fileInfo.write("{:<16f}".format(travel[0]))
                fileInfo.write("   ")
                fileInfo.write("{:<16f}".format(travel[1]))
                fileInfo.write("   ")
                fileInfo.write("{:<16f}".format(travel[2]))
                fileInfo.write("   ")
                fileInfo.write("{:<16f}".format(time.time()))
                fileInfo.write("   ")
                fileInfo.write("\n")
                self.ve(travel[0], travel[1], travel[2], 0)

                #raw_input("Press Enter to continue...")
                # Equation for time determination is
                # time = 0.4363*distance + 1.94
                # from empiric data taken above
                distance = np.sqrt(travel[0]**2 + travel[1]**2 + travel[2]**2)
                timeToSleep = 2.5363 * distance + 2.34
    ...
    def ve(self, x, y, z, azi):
        self.update(cmd=moverACmd(x, y, z, azi))
        # self.wait(1)
        # try:
        #    while self.movementState == None:
        #        print self.position, self.positionGPS
        #    print ("isdhfisdf")
        # except (KeyboardInterrupt, SystemExit):
        #    self.update(cmd = movePCMDCmd(True,0,0,0,0))
    ...
    def moveCamera(self, tilt, pan):
        ...
        # maybe move this to parse data, drone should confirm that
        self.cameraTilt, self.cameraPan = tilt, pan
...
def testManualControlException(robot):
    ...
    try:
        ...
        if robot.flyingState is None or robot.flyingState == 1:  # taking off
            # unfortunately it is not possible to land during takeoff for
            # ARDrone3 :(
            robot.emergency()
        robot.land()
